...
class QuicConnectionProtocol(asyncio.DatagramProtocol):
    ...
    def transmit(self, counter = 0, n_request_migration = 0, interval_migration = 0) -> None: #DEBUG2 TEST* DEBUG V2* PERF EV AUTOMATION* DEBUG V3*
        """
        ...
        # send datagrams
        for data, addr in self._quic.datagrams_to_send( counter, n_request_migration, interval_migration, now=self._loop.time()):   #DEBUG2 TEST* DEBUG V2* PERF EV AUTOMATION* DEBUG V3*
