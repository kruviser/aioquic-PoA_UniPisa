...
POOL_MIGRATION_ADDRESS_FRAME_CAPACITY = 1 + 4   #DEBUG2*
...
class QuicConnection:
    ...
    def datagrams_to_send(self, counter: int, n_request_migration: int, interval_migration: int, now: float) -> List[Tuple[bytes, NetworkAddress]]:  #DEBUG2 TEST DEBUG V2* PERF EV AUTOMATION* DEBUG V3*      
        ...
        for datagram in datagrams:
           ...
           #DEBUG2*********************
            if self._is_client and self._handshake_complete and self._loss._pto_count > 0:   #Max n probe attempt usually 4 or little more due to congestion control new reno and idle timeout
                
                #Container migration comparison
                print(self._loss._pto_count)
                local_arr = []
                for s_m in self._pool_addr_server_migration:
                    if not s_m.probed:
                        local_arr.append(s_m.addr)
                if self._cont_old_server_attempt == 1:
                    local_arr.append(network_path.addr)
                shuffle(local_arr)
                print("Probe a possible server address:")
                print(local_arr[0])
                ret.append((datagram, local_arr[0]))
                if (local_arr[0] == network_path.addr):
                    self._cont_old_server_attempt = 0
                else:
                    for s_m_check in self._pool_addr_server_migration:
                        if(local_arr[0] == s_m_check.addr):
                            s_m_check.probed = True
                            break
                if(len(local_arr) == 1):
                    self._reset_probing()
    ...
    def _update_pool_addr_server(self, network_path: QuicNetworkPath) -> None:
        
        self._cont_old_server_attempt = 1
        for server_migration_path in self._pool_addr_server_migration:
            server_migration_path.probed = False
            if server_migration_path.addr == network_path.addr:
                server_migration_path.addr = self._network_paths[0].addr

    def _reset_probing(self) -> None:
        cont = 0
        for server_migration_path in self._pool_addr_server_migration:
            if server_migration_path.probed:
                cont += 1
        if(cont == len(self._pool_addr_server_migration)):
            for s_m in self._pool_addr_server_migration:
                s_m.probed = False
                self._cont_old_server_attempt = 1
    ...
    def _handle_ack_frame(
        ...
        c = 0
        for s_m in self._pool_addr_server_migration:
            if s_m.addr == context.network_path.addr:
                c += 1
        if c == len(self._pool_addr_server_migration): 
            self._reset_probing()   #Reset probing parameters
        #DEBUG2**
    ...
    def _handle_crypto_frame(
        ...
        if event is not None:
            ...
            if not self._handshake_complete and self.tls.state in [
                ...
                #DEBUG2*********************
                #Add server migration's addresses to the pool
                if not self._is_client:
                    self._replenish_server_migration_addr()
    ...
    def _handle_pool_migration_addr_frame(
        self, context: QuicReceiveContext, frame_type: int, buf: Buffer
    ) -> None:
        """
        Handle a POOL SERVER MIGRATION ADDRESS frame.
        """

        print("PROCESS POOL SERVER MIGRATION ADDRESS") #DEBUG2*
        ...
        #Create QuicServerMigrationPath
        server_migration_path = QuicServerMigrationPath(addr)

        #Adding network path to pool of server migration's addresses
        if server_migration_path not in self._pool_addr_server_migration and len(self._pool_addr_server_migration) != self._max_addr_pool_server_migration:
            self._pool_addr_server_migration.append(server_migration_path)
        
        # log frame
        if self._quic_logger is not None:
            context.quic_logger_frames.append(self._quic_logger.encode_pool_migration_address_frame(data))
    ...
    def _on_pool_migration_addr_delivery(
        self, delivery: QuicDeliveryState, server_migration_path: QuicServerMigrationPath
    ) -> None:
        """
        Callback when a POOL SERVER MIGRATION'S ADDRESS frame is acknowledged or lost.
        """

        if delivery != QuicDeliveryState.ACKED:
            idx = self._pool_addr_server_migration.index(server_migration_path)
            if idx:
                self._pool_addr_server_migration[idx].was_sent = False
    ...
    def _replenish_server_migration_addr(self) -> None:
        """
        Add server migration's addresses to the pool
        """
        counter = 0
        while counter < self._max_addr_pool_server_migration:
            ip = "172.16.4."
            if counter == 0:
                ip = ip + str(232)
            else:
                num = random.randint(60, 254)
                ip = ip + str(num)
    ...
    def _write_application(
        ...
        while True:
            ...
            if self._handshake_complete:
                ...
                # POOL OF SERVER MIGRATION'S ADDRESSES
                if not self._is_client:
                    for server_migration_path in self._pool_addr_server_migration:
                        if not server_migration_path.was_sent:
                            self._write_pool_migration_addr_frame(
                                builder=builder, server_migration_path=server_migration_path
                            )
    ...
    def _write_pool_migration_addr_frame(
        self, builder: QuicPacketBuilder, server_migration_path: QuicServerMigrationPath
    ):

        print("WRITE POOL SERVER MIGRATION ADDRESS FRAME") 

        #server_path.addr has the following format: ('::ffff:192.168.178.55', 4433, 0, 0)
        ip = server_migration_path.addr[0].split(":")[3]
        ...
        buf = builder.start_frame(
            QuicFrameType.POOL_MIGRATION_ADDRESS,
            capacity=POOL_MIGRATION_ADDRESS_FRAME_CAPACITY, 
            handler=self._on_pool_migration_addr_delivery,
            handler_args=(server_migration_path,),
        )   
        buf.push_uint32(ip_int)

        server_migration_path.was_sent = True

        self._logger.debug(
            "Sending POOL SERVER MIGRATION ADDRESS in packet %d",
            builder.packet_number,
        )

        # log frame
        if self._quic_logger is not None:
            builder.quic_logger_frames.append(self._quic_logger.encode_pool_migration_address_frame(data=ip_int))
